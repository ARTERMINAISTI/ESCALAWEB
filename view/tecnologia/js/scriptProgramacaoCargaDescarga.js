...
$(document).ready(function () {
    $('#reqtablenew td').dblclick(function () {
        ...
        $('#inserirAnexoCargaDescarga').removeClass('display');
    });
    ...
    $('#reqtablenewMobile td').dblclick(function () {
        ...
        $('#inserirAnexoCargaDescargaMobile').removeClass('display');
    });
});
...
function manterOcorrencia() {
    ...
    jQuery.ajax({
        ...
        data: {
            ...
            motivoOcorrenciaHandle: $('#motivoOcorrenciaHandle').val(),
            anexos: $('#anexos').val()
        },
...
function inserirAnexo() {
    if ($('#anexos').val() !== '' && handleOcorrencia != 0) {
        var input = document.getElementById('anexos');
        for (var x = 0; x < input.files.length; x++) {    
            var form_data = new FormData();
            form_data.set('handleOcorrencia', handleOcorrencia)
            form_data.append('files', $('#anexos').prop('files')[x]);
            jQuery.ajax({
                type: "POST",
                ...
                processData: false,
                success: function (data) {
                    mensagem = mensagem + '<br> Anexos cadastrados.' 
                },
                error: function (data) {
                    msg = 'Erro ' + data.responseJSON.code + ': ' + data.responseJSON.message
                    mostrarErro(msg);
...
function InserirAnexoCargaDescargaFun() {
    var carregamento = getHandleCarregamento();
    window.location.href = '../../view/armazenagem/AnexoCargaDescarga.php?cargaDescarga=' + carregamento
...
function OcorrenciaProgramacaoCargaDescargaFun() {
    ...
    $('#obs').val('');
    $('#motivoOcorrencia').val('');
    $('#motivoOcorrenciaHandle').val('');
    ...
    jQuery.ajax({
        ...
        success: function (data) {
            ...
            if (data.ERRO !== '') {
                ...
                $('#motivoOcorrencia').val(data.MOTIVOOCORRENCIA);
                $('#motivoOcorrenciaHandle').val(data.MOTIVOOCORRENCIAHANDLE);
...
function motivoOnPesquisar() {
    jQuery.ajax({
        type: "POST",
        url: "../../controller/operacional/BaixarMotivoOcorrenciaCargaDescarga.php",
        dataType: 'json',
        data: {handle: getHandleCarregamento()},
        success: function (data) {
            $('#motivoOcorrencia').autocomplete({
                select: function (event, ui) {
                    $("#motivoOcorrenciaHandle").val(ui.item.id);
...
function clickdownMotivoOcorrencia() {
    $('#motivoOcorrencia').autocomplete('option', 'minLength', 0);
    $('#motivoOcorrencia').autocomplete('search', $('#motivoOcorrencia').val());
}
// Recupera Tipo
$(document).ready(function () {
