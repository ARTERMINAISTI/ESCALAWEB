...
<body>
                            ...
                            <?php if($relacionamento["ANEXOS"]){ ?>
                                ...
                                <?php
                                    foreach($relacionamento["ANEXOS"] as $anexo){
                                        ...
                                        echo "<span><a class=\"link_download\" id=". base64_encode($dadosOrdem["CHAVE"] ."|". $anexo["ANEXO"]) ."  >" . $anexo["ANEXO"] . "</a></span>; ";
                                    }
                                ?>
                                </div>
                            <?php } ?>
    ...
    <script>
        function DonwloadImagem(event){
            let varloDecoded = atob(event.target.id)

            let array = varloDecoded.split("|");
            
            $.ajax({
                url: "../../model/relacionamento/getAnexo.php",
                method: "GET",
                dataType: "text",
                data: {
                    "atendimento": array[0],
                ...
                success: function (retorno) {
                    ...
                    if(userAg.indexOf('Android') > -1) {
                        let arquivoRelacionamento = atob(retorno.trim());

                        Android.saveFile(arquivoRelacionamento, array[1]);
                    } else {
                        var a = document.createElement("a"); //Create <a>
                        a.href = "data:image/png;base64," + retorno.trim(); //Image Base64 Goes here
                        a.download = array[1]; //File name Here
        ...
        let links = $(".link_download");
        
        for(var i = 0; i < links.length; i++)
        {
            links[i].addEventListener("click", DonwloadImagem);
        }        
    </script>
</body>
</html>