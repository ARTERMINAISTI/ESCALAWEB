...
$("#pesquisa").keyup(function(event) {
    if($(this).val().length >= 3) {
        ...
        $(".tddata").each(function() {
            if($(this).html().toUpperCase().indexOf(pesquisa.toUpperCase()) > -1) {
                $(this).parent().show();
            } else {
                $(this).parent().hide();
            }
...
function BaixarOnClick() {
    ...
    if (romaneioItem.length == 0){        
        ...
        if(operacoes.indexOf('0') > -1){
            ...
            //Apenas controles de chegada/saida do local
            botaoBaixarItemGrid();    
        }
    }
}
...
function BotaoBaixarItemOnClick(){
    ...
    if (romaneioItem.length == 0){        
        ...
        campoFile.setAttribute("accept", "image/*");        
        ...
        $("div.dropzone").dropzone({ 
            ...
            success: function (file, retorno) {  
                let jsonRetorno = JSON.parse(retorno.trim());
                swal({
                    title: "Itens baixados!",
...
function criarOcorrencias(){     
    ...
    let acao = $('#tipoOcorrencia option:selected').attr('acao');        
    ...
    $.ajax({
        ...
        success: function (retorno, status, request) {  
            ...
            if(userAg.indexOf('Android') > -1) {
                Android.saveFile(retorno.Arquivo, "Impressao_"+handle);
            } else {
                let pdfWindow = window.open("")
                pdfWindow.document.write(
                    "<iframe width='100%' height='100%' src='data:application/pdf;base64, " +
                    encodeURI(retorno.Arquivo) + "'></iframe>"
                )
            }                        
