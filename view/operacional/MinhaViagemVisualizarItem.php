...
$sqlLocal = "SELECT DISTINCT B.APELIDO,
                    A.SEQUENCIA,
                    CASE WHEN (SELECT COUNT(X.HANDLE)
                                 FROM OP_VIAGEMROMANEIOITEM X
                                WHERE X.ROMANEIOLOCAL = A.HANDLE
                                  AND X.STATUS NOT IN (3,4)) = 0 THEN 'S' ELSE 'N' END EHENCERRADO,
                        ...
                        <tr>
                            <td class="tdcheck">
                                ...
                                <?php
                                    ...
                                    <input class="big-checkbox item" type="checkbox" local="<?= $dataSetLocal['HANDLE'] ?>" item="<?=$dataSetItem['HANDLE']?>" name="viagemitemhandle" id="viagemitemhandle" operacao="<?= $dataSetLocal['OPERACAO']?>" value="<?= $dataSetItem['HANDLE'] ?>">
                                <?php } ?>
                            </td>
                            <td class="tddata" style="padding-top: 3px;" onclick="trViagemItemOnClick(<?= $dataSetItem['TIPODOCUMENTO'] ?>,<?= $dataSetItem['HANDLE'] ?>)">                            
                                ...
                                <small class="floatRight"><?php echo "Originário: ".$dataSetItem['ORIGINARIOS']; ?></small>                                
    ...
    $sqlItem = "SELECT A.NUMERO,
                    ...
                    SELECT  B1.NUMERO,
                      ...
                      INNER JOIN OP_VIAGEMROMANEIOITEM A2 ON A2.DOCUMENTOTRANSPORTE = A.HANDLE
                       LEFT JOIN OP_OCORRENCIA L ON L.ROMANEIOITEM = A2.HANDLE
                                                AND L.ACAO IN (15, 6, 4)
                                                AND L.STATUS = 4
                       LEFT JOIN OP_VIAGEM M ON M.HANDLE = A2.VIAGEM
                       LEFT JOIN OP_STATUSROMANEIOITEM N ON N.HANDLE = A2.STATUS
                      INNER JOIN GD_ORIGINARIO O ON O.HANDLE = A2.ORIGINARIO
                      WHERE A2.ROMANEIOLOCAL = '$local'
                        AND A2.STATUS NOT IN (4)";
