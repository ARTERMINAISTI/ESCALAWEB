...
$queryRastreamentoUltimaEtapa = "SELECT TOP 1 HANDLE, DATA, ETAPA, OBSERVACAO
								FROM (
								SELECT  A.HANDLE HANDLE, A.DATA DATA, A.TITULO ETAPA, A.OBSERVACAO OBSERVACAO
...
$queryRastreamentoDocumento = "SELECT A.HANDLE, 
						...
						LEFT JOIN MS_FILIAL B6 ON A.FILIAL = B6.HANDLE
						LEFT JOIN MS_PESSOA B7 ON B6.PESSOA = B7.HANDLE
						WHERE EXISTS (  
												  SELECT X.HANDLE             
												  FROM RA_PEDIDOMOVIMENTACAO X            
												  WHERE X.PEDIDO = '" . $handleRastreamento . "'
