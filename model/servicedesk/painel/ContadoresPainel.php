...
$filialDesenv = getFilialPessoaLogadaDesenvolvimento($connect);

$condicaoFilial = (($filial <> 0) && ($filial <> "")) ? " AND A.FILIAL IN (".$filial.") " : " ";
$condicaoFilialDesenv = (($filialDesenv <> 0) && ($filialDesenv <> "")) ? " AND A.FILIAL IN (".$filialDesenv.") " : " ";

$queryAtendimento = "SELECT ( SELECT COUNT(A.HANDLE)
                                FROM SD_ORDEM A
                                WHERE A.STATUS NOT IN (4, 5) -- Encerrado e Cancelado
                                  ".$condicaoSistema."
                                  ".$condicaoFilial."
                            ...
                            ( SELECT SUM(A.SUPORTE)
                                ...
                                WHERE A.DATA BETWEEN '$ontemInicio' AND '$ontemFim'
                                  ".$condicaoSistema."
                                  ".$condicaoFilial."
                            ) QTDINICIALATENDIMENTO,

                            ( SELECT COUNT(A.HANDLE) FROM SD_ORDEM A
                                WHERE A.DATA BETWEEN '$hojeInicio' AND '$hojeFim'
                                  ".$condicaoSistema."
                                  ".$condicaoFilial."
                            ...
                            ( SELECT COUNT(A.HANDLE) FROM SD_ORDEM A
                                WHERE A.STATUS NOT IN (5) -- Cancelado
                                ".$condicaoSistema."
                                ".$condicaoFilial."
...
$queryDesenvolvimento = "SELECT ( SELECT COUNT(A.HANDLE)
                                ...
                                ( SELECT COUNT(A.HANDLE)
                                    FROM SD_ORDEM A
                                    WHERE A.TIPO NOT IN (2) -- Não pega melhoria
                                    ".$condicaoSistema."
                                    ".$condicaoFilialDesenv."
                                      AND A.STATUS NOT IN (5) -- Cancelado
                                      AND EXISTS (SELECT X1.HANDLE FROM SD_ORDEMENCAMINHAMENTO X1
