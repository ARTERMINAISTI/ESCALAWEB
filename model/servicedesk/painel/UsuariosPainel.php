...
function getListaUsuarios($connect)
{
    ...
    $filial = getFilialPessoaLogada($connect);
    $condicaoFilial = (($filial <> 0) && ($filial <> "")) ? " AND B.PESSOAFILIAL IN (".$filial.") " : " ";
    
    $condicaoUnidade = "";
    if ($unidadeNegocio['HANDLE'] <> 0){
        $condicaoUnidade = " AND B.PESSOAUNIDADENEGOCIO = '" . $unidadeNegocio['HANDLE'] . "' ";
    } 
...
function getFilialPessoaLogada($connect)
{
    $query = "SELECT B.PESSOAFILIAL
                FROM MS_USUARIO A
               INNER JOIN MS_PESSOA B ON A.PESSOA = B.HANDLE
               WHERE A.HANDLE = " . $_SESSION["handleUsuario"];
...
function getValoresUsuario($usuario, $unidadeNegocio, $connect)
{
    ...
    $filial = getFilialPessoaLogada($connect);
    $condicaoFilial = (($filial <> 0) && ($filial <> "")) ? " AND A.FILIAL IN (".$filial.") " : " ";
    ...
    if ($unidadeNegocio === 'armazenagem') {
        ...
        $condicaoSistema = " AND A.SISTEMA IN (".$unidadeNegocio.") ";
        ...
        $condicaoSistema = " AND A.SISTEMA IN (".$unidadeNegocio.") ";
    } elseif ($unidadeNegocio === 'consultoria') {
    $unidadeNegocio = implode(',', [
        1, //ERP
        3, //TMS
        4, //WMS
...
//            10  // RH
    ...
    if ($usuario["ATIVIDADE"] === 'A') {
        ...
        $sqlRetra = "SELECT COUNT(A.HANDLE)
                        FROM SD_ORDEM A
                        WHERE EXISTS (SELECT * FROM SD_ORDEMENCAMINHAMENTO X1 WHERE X1.ORDEM = A.HANDLE AND X1.MOTIVOENCAMINHAMENTO IS NOT NULL)
                        ".$condicaoFilial."
                        ".$condicaoSistema."
        ...
        $sqlRetra = "SELECT COUNT(A.HANDLE)
                        ...
                        ".$condicaoSistema."
                        ".$condicaoFilial."
                        AND A.RESPONSAVEL = $handleUsuario
                        AND EXISTS (SELECT * FROM SD_ORDEMAPONTAMENTO X1 WHERE X1.ORDEM = A.HANDLE AND X1.RECURSO = $handleUsuario AND X1.DATA BETWEEN '$hojeInicio' AND '$hojeFim')";
    ...
    $query = "SELECT (SELECT COUNT(A.HANDLE)
                FROM SD_ORDEM A
                WHERE EXISTS ( SELECT X1.HANDLE FROM SD_ORDEMENCAMINHAMENTO X1
...
function pegaUsuarioComCritico($unidadeNegocio, $usuarios, $connect)
{
    ...
    $condicaoFilial = (($filial <> 0) && ($filial <> "")) ? " AND (CASE WHEN A.DEPARTAMENTO NOT IN (1, 5) THEN ".$filial." ELSE 0 END = CASE WHEN A.DEPARTAMENTO NOT IN (1, 5) THEN A.FILIAL ELSE 0 END
                                                                   OR  CASE WHEN A.DEPARTAMENTO NOT IN (1, 5) THEN ".$filial." ELSE 0 END = CASE WHEN A.DEPARTAMENTO NOT IN (1, 5) THEN F.PESSOAFILIAL ELSE 0 END)" : " "; //DESENV, QUALIDADE
    $query = "SELECT SUBSTRING(G.NOME, 1, 1) ATIVIDADE,
                               ...
                               null HRS,
                               null DEV,
                               ( SELECT SUM(DATEDIFF(minute, CONVERT(VARCHAR(5), X1.INICIO, 108), CONVERT(VARCHAR(5), X1.TERMINO, 108))) FROM SD_ORDEMAPONTAMENTO X1 WHERE X1.ORDEM = A.HANDLE) TOTALAPONTA,
                               null TOTAL,
                               F.APELIDO CLIENTE,
                               A.HANDLE SD,
...
function getFilialPessoaLogadaDesenvolvimento($connect)
{
    $query = "SELECT B.PESSOAFILIAL, D.FILIALMATRIZ
                FROM MS_USUARIO A
               INNER JOIN MS_PESSOA B ON A.PESSOA = B.HANDLE
                LEFT JOIN MS_FILIAL C ON C.HANDLE = B.PESSOAFILIAL
                LEFT JOIN MS_EMPRESA D ON D.HANDLE = C.EMPRESA
               WHERE A.HANDLE = " . $_SESSION["handleUsuario"];
    ...
    if ($retorno['PESSOAFILIAL'] <> $retorno['FILIALMATRIZ']){
