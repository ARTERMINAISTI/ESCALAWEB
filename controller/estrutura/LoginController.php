...
setcookie("dataProximaTentativaLogin", "AAAA");
...
if (isset($entrar)) {
    ...
    $QueryEmpresa->bindParam(':USUARIO', $handleUsuario, PDO::PARAM_STR);
    ...
    $QueryFilial->bindParam(':USUARIO', $handleUsuario, PDO::PARAM_STR);    
    $QueryFilial->execute();
    ...
    $QueryEmpresaPermissao = $connect->prepare("SELECT A.HANDLE, 
											   (SELECT COUNT(B.HANDLE) FROM MS_FILIAL B WHERE B.EMPRESA = A.HANDLE AND B.STATUS <> 6) TEMFILIAL 
												  FROM MS_EMPRESA A 
									             WHERE A.STATUS <> 6 
                                                   AND (EXISTS(SELECT C.HANDLE FROM MS_USUARIOEMPRESA C WHERE C.EMPRESA = A.HANDLE AND C.USUARIO = (:USUARIO1))
									           OR NOT EXISTS(SELECT D.HANDLE FROM MS_USUARIOEMPRESA D WHERE D.USUARIO = (:USUARIO2)))
											  ");
    $QueryEmpresaPermissao->bindParam(':USUARIO1', $handleUsuario, PDO::PARAM_STR);    
    $QueryEmpresaPermissao->bindParam(':USUARIO2', $handleUsuario, PDO::PARAM_STR);    
    ...
    $empresa = $rowEmpresaPermissao['HANDLE'];
    ...
    if ($handleUsuario > 0) {
        ...
        $painelTelaInicial = $usuario->getPainelIndicadorTelaInicial();

        if ($painelTelaInicial > 0) {
            $_SESSION['paginaPrincial'] = "../../view/inteligencianegocio/visualizarpainel.php?" . $painelTelaInicial;
        }
        else {
            $_SESSION['paginaPrincial'] = "../../view/estrutura/index.php";
        }
        
        setcookie("ultimologinWeb", strtolower($login), time()+3600*24*30, '/');

        header('Location: ' . $_SESSION['paginaPrincial']);
    }
    else{
        if (($handleUsuario == '0') || ($handleUsuario == '')) {
